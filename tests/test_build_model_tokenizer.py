...
def test_build_model_tokenizer():
    ...
    for model_checkpoint, tokenizer_class in model_tokenizer_class:
        ...
        for num_eos_tokens in [ 1, 4 ]:
            ...
            assert tokenizer.num_eos_tokens == num_eos_tokens, f"num_eos_tokens is {tokenizer.num_eos_tokens} but should be {num_eos_tokens} model {model_checkpoint},"
            assert isinstance(tokenizer, tokenizer_class), f"tokenizer is {tokenizer} but should be {tokenizer_class}"

            if num_eos_tokens != 1:
                assert '<end_of_sentence>' not in tokenizer.get_vocab(), f"<end_of_sentence> is in the vocab for model {model_checkpoint}"
