    ...
    for seq_length in [4096]:
        ...
        for num_eos_tokens in [1, 2]:
            ...
            for pretrained_model_name in ["unsloth/Llama-3.2-1B"]:
                ...
                script = f"bash -c 'cd {workdir} && /workspace-SR004.nfs2/d.tarasov/envs/tokens_pruning/bin/python scripts/tokenize_fineweb_edu.py --pretrained_model_name {pretrained_model_name} --with_eos_token --num_eos_tokens {num_eos_tokens} --max_length {seq_length}'"
                print("\n\n", script)
                if dry:
