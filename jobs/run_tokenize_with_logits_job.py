        ...
        for num_eos_tokens in [8]:
            ...
            for shard_index in range(48):
                ...
                for pretrained_model_name in ["unsloth/Llama-3.2-1B"]:
                    ...
                    script = f"bash -c 'cd {workdir} && /workspace-SR004.nfs2/d.tarasov/envs/sentence_attention/bin/python scripts/tokenization/tokenize_with_logits_fineweb_edu.py --pretrained_model_name {pretrained_model_name} --max_length {seq_length} --num_shards {num_processing_shards} --shard_index {shard_index} '"
                    print("\n\n", script)
                    if dry:
                    ...
                    result = client.run_job(
                        payload={
                            "script": script,
                            "job_desc": f"Tokenize with logits fineweb EOS model={pretrained_model_name} num_eos_tokens={num_eos_tokens} seq_length={seq_length} shard_index={shard_index} #{author_name} #rnd #multimodal #notify_completed @mrsndmn",
