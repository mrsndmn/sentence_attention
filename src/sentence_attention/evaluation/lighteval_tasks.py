...
def text_dataset_prompt(line, task_name: str = None):
    return Doc(
        task_name=task_name,
        query=line["text"],
        choices=[""],
...
def text_dataset_prompt_with_end_of_sentence(line, task_name: str = None):
    return Doc(
        task_name=task_name,
        query=line["text"].replace(". ", ". <end_of_sentence> "),
        choices=[""],
        gold_index=0,
    )
...
def hellaswag_prompt(line, task_name: str = None):
    ...
    return Doc(
        ...
        # choices=[preprocess(ending).strip() for ending in line["endings"]],
...
for suffix, prompt_function in [("", text_dataset_prompt), ("_with_end_of_sentence", text_dataset_prompt_with_end_of_sentence)]:
    wiki_text_task = LightevalTaskConfig(
        name=f"wikitext_103{suffix}",
        prompt_function=prompt_function,
        # hf_repo="Salesforce/wikitext",
        # hf_subset="wikitext-103-raw-v1",
        hf_repo="lighteval/wikitext_103",
        hf_subset="default",
        evaluation_splits=["test"],
        metric=[Metrics.target_perplexity],
    )
    tiny_stories_task = LightevalTaskConfig(
        name=f"tiny_stories{suffix}",
        prompt_function=prompt_function,
        # hf_repo="Salesforce/wikitext",
        # hf_subset="wikitext-103-raw-v1",
    ...
    PPL_TASKS.append(wiki_text_task)
    PPL_TASKS.append(tiny_stories_task)
...
COMMON_SENSE_REASONING_TASKS = [
    LightevalTaskConfig(
        ...
        hf_subset="default",
        trust_dataset=True,
        metric=[Metrics.loglikelihood_acc, Metrics.loglikelihood_acc_norm_nospace, Metrics.loglikelihood_acc_token_norm],
    ),
    LightevalTaskConfig(
        name="winogrande",
        prompt_function=default_prompts.winogrande,
        hf_repo="winogrande",
    ...
    LightevalTaskConfig(
        name="piqa",
        prompt_function=default_prompts.piqa_harness,
        hf_repo="piqa",
    ...
    LightevalTaskConfig(
        name="openbookqa",
        prompt_function=default_prompts.openbookqa,
        hf_repo="openbookqa",
        hf_subset="main",
        trust_dataset=True,
        metric=[Metrics.loglikelihood_acc, Metrics.loglikelihood_acc_norm_nospace],
    ...
    LightevalTaskConfig(
        name="arc:easy",
        prompt_function=default_prompts.arc,
        hf_repo="ai2_arc",
        hf_subset="ARC-Easy",
        evaluation_splits=["test"],
        generation_size=1,
    ...
    LightevalTaskConfig(
        name="trivia_qa",
        prompt_function=default_prompts.triviaqa,
...
GSM8K = LightevalTaskConfig(
    name="gsm8k",
    prompt_function=default_prompts.gsm8k,
    hf_repo="gsm8k",
    hf_subset="main",
...
MATH_TASKS = [
    LightevalTaskConfig(
        name=f"math:{subset}",
        prompt_function=default_prompts.math,
...
for answer_type in ("mc", "cloze"):
    ...
    if prompt_function == "mmlu_mc_prompt":
        prompt_function = mmlu_mc_prompt
    elif prompt_function == "mmlu_cloze_prompt":
        prompt_function = mmlu_cloze_prompt
    else:
        raise ValueError("inknown prompt function")
