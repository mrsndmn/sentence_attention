...
def cumsum_exclusive(t, dim=-1):
    num_dims = len(t.shape)  # noqa: F841
    num_pad_dims = -dim - 1
...
def safe_one_hot(indexes, max_length):
    ...
    if indexes.numel() == 0:
        return F.one_hot(indexes, max_length)
...
class Top2Gating(nn.Module):
    ...
    def forward(self, x, importance=None, attention_mask=None):
        *_, b, group_size, dim = x.shape
        ...
        mask_1 = F.one_hot(index_1, num_gates).to(x.dtype)
        density_1_proxy = raw_gates

        if importance is not None:
            equals_one_mask = (importance == 1.0).to(x.dtype)
        ...
        mask_2 = F.one_hot(index_2, num_gates).to(x.dtype)

        if importance is not None:
            greater_zero_mask = (importance > 0.0).to(x.dtype)
        ...
        if policy == "all":
            ...
            mask_2 *= (gate_2 > threshold).to(x.dtype)
        elif policy == "random":
            probs = torch.zeros_like(gate_2).uniform_(0.0, 1.0)
            mask_2 *= (probs < (gate_2 / max(threshold, self.eps))).to(x.dtype).unsqueeze(-1)
        else:
        ...
        mask_1 *= (position_in_expert_1 < expert_capacity_f).to(x.dtype)
        # [batch, experts]
        ...
        mask_2 *= (position_in_expert_2 < expert_capacity_f).to(x.dtype)
...
class MoE(nn.Module):
    def __init__(
        ...
        self.experts = default(
            experts, lambda: Experts(dim, num_experts=num_experts, hidden_dim=hidden_dim, activation=activation)
        )
        self.loss_coef = loss_coef

    def forward(self, inputs, attention_mask):

        b, n, d, e = *inputs.shape, self.num_experts  # noqa: F841
        dispatch_tensor, combine_tensor, loss = self.gate(inputs, attention_mask=attention_mask)
